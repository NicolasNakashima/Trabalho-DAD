...
const correctAnswers = [
    ...
    ["docker run", "python:3.9", "--name", "py", "-it", "bash"],
    ["docker-compose", "up"],
...
function checkAnswer(event) {
    ...
    if(verificacaoResposta.children.length == 0) {
        ...
        for(let i = 0; i < caixaResposta.children.length; i++) {
            if(caixaResposta.children[i].textContent.trim() == correctAnswersExercise[i]) {
                btn.remove()
                acertos++
...
function reloadAnswer(event) {
    ...
    for(; caixaResposta.children.length > 0;) {
        verificacaoResposta.appendChild(caixaResposta.children[caixaResposta.children.length - 1])
    }

    for(let i = 0; i < boxResult.children.length; i++) {
        if(boxResult.children[i].classList.contains("wrong") || boxResult.children[i].classList.contains("correct")) {
